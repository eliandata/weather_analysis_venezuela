...
temp_prec_city = (
    ...
    .groupby(['city', 'Year'])[['Temperature','Precipitation']].mean()
    .reset_index()
    .sort_values(by='Year')  
    .reset_index(drop=True)
)

corr_var= temp_prec_city.corr()

fig = px.imshow(corr_var, text_auto=True, aspect="auto")
st.subheader('Correlation Temperature vs Precipitation over the years')
st.plotly_chart(fig)
